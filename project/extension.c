//By:Yun peng zou
#include <stdio.h>
#include <stdlib.h>
#include <sys/stat.h>
#include <string.h>
#include <pthread.h>
#include <semaphore.h>
#include <mqueue.h>
#include <errno.h>
#include <math.h>
#include "dlab.h"
	
	pthread_t controlthread;
	pthread_t runm;
	sem_t data_avail;
	#define MAXS 5000
	float thetadeg[MAXS];
	float refdeg [MAXS];
	float thetarad[MAXS];
	float refrad [MAXS];
	float thetapre [MAXS];
	float thetapre [MAXS];
	double Kp = 30; 		   // Initialize Kp to 30.
double run_time = 3.0;   // Set the initial run time to 3 seconds.
double Fs = 200.0; 	   // Set the initial sampling frequency to 200 Hz.
int motor_number = 5; // See lab_layout.pdf for your motor_number.
int no_of_samples;
int firsttime = 0;
int go;

double Ti = 0.0245;
double Td = 0.006125;
double N = 1000;
double Tt = 0.01;
int stop=0;
double satblk (double input_voltage)
{

	double output_voltage;

	if (input_voltage > 1.4) {

		output_voltage = 1.4;
	}

	else if (input_voltage < -1.4) {
		output_voltage = -1.4;
	}
	else  {
		output_voltage = input_voltage;
	}

	return output_voltage;
}
	
	void *Control(void *arg)
{
	double ek;
	double uk = 0;
	float motor_position;

	
	double P;
	double I;
	double D;
	double ekpre = 0;
	double dpre = 0;
	double ipre = 0;
	double T = 1/Fs;

	double a;
	
	double a_minus1 = 0;
	double v;


	while(stop == 0){
	int k = 0;

	go = 0;
	no_of_samples = (int)(3*Fs);
	T = 1/Fs;
	while (k < no_of_samples) {
		
		sem_wait(&data_avail);
		motor_position = EtoR(ReadEncoder());

		// calculate tracking error
	   ek = refrad[k] - motor_position;

		// calculate control value


		P = Kp*ek;
		I = ipre + (Kp/Ti)*(ekpre)*(T) + (1/Tt)*(a_minus1)*(T);
		D = (Td/(N*T + Td))* dpre + ((Kp*Td*N)/(N*T + Td))*(ek - ekpre);
		v = P + I + D;

		uk = satblk(v);
		a = uk-v;

		DtoA(VtoD(uk));
		thetarad[k] = motor_position;
		thetadeg[k] = motor_position/M_PI*180;

		ipre = I;
		dpre = D;
		ekpre = ek;
		a_minus1 = a;

		k++;
		
	}
	
	go = 1;
	
	}
	pthread_exit(NULL);
}

void *runmotor(void *arg)
{

		char selection;
		double inp;
		
		printf("Second Menu selections: \n");
		printf("f: Change value of sample freq \n");
		printf("p: Change value of Kp \n");
		printf("i: Change value of Ti \n");
		printf("d: Change value of Td \n");
		printf("n: Change value of N \n");
		printf("s: Stop \n");



	while (stop == 0) {


		printf("Choose your selection \n");
		scanf(" %c", &selection);  //Get the input from the user

		switch (selection) {


		case 'f':

	
			printf("What is the new value of the sample frequency in Hz? \n");
			scanf("%lf", &inp);  //Get the input from the user
			getchar();


			while (go == 0) { }
			if (go == 1) {
			Fs = inp; 
			no_of_samples = (int)(3*Fs);
			}


			break;



		case 'p':

			// Prompt user for new value of Kp;
			printf("What is the new value of Kp? \n");
			scanf("%lf", &inp);  //Get the input from the user
			getchar();
		
			while (go == 0) { }
			if (go == 1) {
			Kp = inp; 
			}
		

			break;


		case 'i':

			printf("What is the new value of Ti? \n");
			scanf("%lf", &inp);  //Get the input from the user
			getchar();
		
			while (go == 0) { }
			if (go == 1) {
			Ti = inp; 
			}
		
			break;


		case 'd':

			printf("What is the new value of Td? \n");
			scanf("%lf", &inp);  //Get the input from the user
			getchar();
			
			while (go == 0) { }
			if (go == 1) {
			Td = inp; 
			}
			
			break;


		case 'n':

			printf("What is the new value of N? \n");
			scanf("%lf", &inp);  //Get the input from the user
			getchar();
			
			while (go == 0) { }
			if (go == 1) {
			N = inp; 
			}
			

			break;

		case 's':

			printf("Stopping motor \n");
			stop = 0;
			pthread_exit(NULL);
			break;
		
		case 'g':
			plot(thetadeg, refdeg, Fs, no_of_samples, SCREEN, "Response of Motor", "Time (seconds)", "Shaft position (degrees)");
			break;
			
			case 'h':
				plot(refdeg,thetadeg, Fs, no_of_samples, PS, "Response of Motor", "Time (seconds)","Shaft position (degrees)");
			break;
			
		default:
			printf("Sir Error: Invalid selection! \n");

			
		}  
	} 
	


	pthread_exit(NULL);

}

int main(void *arg)
{

	char oper, type;
	float mag, freq, duty;
    int i;
	no_of_samples = (int)(run_time*Fs);
	
	Square(refrad, no_of_samples, Fs, 50*M_PI/180.0, 1, 50);
	Square(refdeg, no_of_samples, Fs, 50, 1, 50);
	
	while(1){
		printf("Please select a option\n");
		scanf(" %c", &oper);
		
		switch(oper){
			case'r':
				printf("Running motor.\n");
				sem_init(&data_avail, 0, 0);
				Initialize(DLAB_SIMULATE, Fs, motor_number);
				if (pthread_create(&controlthread, NULL, &Control, NULL) != 0) {
		       		printf("Error creating thread.\n");
		       		exit(-1);
		  		}
				if (pthread_create(&runm, NULL, &runmotor, NULL) != 0) {
		       		printf("Error creating thread.\n");
		       		exit(-1);
		  		}
				
				pthread_join(runm, NULL);
				pthread_join(controlthread, NULL);
				
				Terminate();
				
				sem_destroy(&data_avail);
				break;
			case 'u':
			printf("Please select a input s for step q for square\n");			
			scanf(" %c", &type);
			
			if (type == 's') {
			//prompt for magnitude of the step
			//set up the step reference input {ref[k]}
				printf("Please enter a magnitude for step input\n");	
				scanf("%f", &mag);
				for(i=0; i<no_of_samples; i++){
					refrad[i] = mag*M_PI/180.0;
					refdeg[i] = mag;
				}
			}
			if (type == 'q') {
				printf("Please select a magnitude in degree\n");
				scanf("%f", &mag);
				printf("Please select a frequency in degree\n");
				scanf("%f", &freq);
				printf("Please select a duty in degree\n");
				scanf("%f", &duty);
				Square(refrad, no_of_samples, Fs, mag*M_PI/180.0, freq, duty);
				Square(refdeg, no_of_samples, Fs, mag, freq, duty);
				
			}
			
			break;

			
			case 'p':
			//Prompt user for new value of Kp;
				printf("Please select a kp\n");
				scanf("%f", &mag);
				Kp = mag;
			break;
			
			case 'f':
			//Prompt user for new value of Kp;
				printf("Please select a sampling frequency\n");
				scanf("%f", &mag);
				
				Fs = mag;
				no_of_samples = (int)(run_time*Fs);
			break;
			
			case 't':

				printf("Please select a time\n");
				scanf("%f", &mag);

				 run_time = mag;
				 no_of_samples = (int)(run_time*Fs);
			break;
			
			case 'i':

				printf("Please select a Ti\n");
				scanf("%f", &mag);
				 Ti = mag;
				 
			break;
			
			case 'd':

				printf("Please select a Td\n");
				scanf("%f", &mag);
				 Td = mag;
				 
			break;
			
			case 'n':

				printf("Please select a N\n");
				scanf("%f", &mag);
				 N = mag;
				 
			break;
			
			case 'g':
			plot(thetadeg, refdeg, Fs, no_of_samples, SCREEN, "Response of Motor", "Time (seconds)", "Shaft position (degrees)");
			break;
			
			case 'h':
				plot(refdeg,thetadeg, Fs, no_of_samples, PS, "Response of Motor", "Time (seconds)","haft position (degrees)");
			break;

			case 'q':
			//We are done!
			exit(0);
			break;
			default:
				printf("Invalid \n" );
			break;
		}
	}
	exit(0);
}
